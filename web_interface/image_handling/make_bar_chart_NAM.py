...
NAM ="NAM"
fh = open('./tmp/%s-%s-%s-map.html' % (NAM,mygene,expression), 'w')
if (bool(myexp) == True):
    ...
    fig.savefig(('./tmp/%s-%s-%s-small.png' % (NAM,mygene,expression)), dpi=int(900 / upper_limit))
    fig.savefig(('./tmp/%s-%s-%s.svg' % (NAM,mygene,expression)), dpi=args.dpi)
    fig.savefig(('./tmp/%s-%s-%s.png' % (NAM,mygene,expression)), dpi=args.dpi)
    ...
    fh.write('<a href="./tmp/%s-%s-%s.png"><IMG SRC="./tmp/%s-%s-%s-small.png" ismap usemap="#points"></a><MAP name="points">' % (
        NAM,mygene,expression,NAM, mygene,expression))
