...
db_file = '/var/www/html/qTeller/web_interface/gene_protein_qt5db'
...
parser.add_argument('--names', dest='names', help="The name of the gene to draw on the x axis")
...
if len(exps) > 0:
    ...
    for x in include_vals:
        if x == '': continue
        if (x in myexp1.keys() or x in myexp2.keys()):
            Source = myexp1[x]['source']

        if x in myexp1.keys():
            t1[x] = myexp1[x]
        else:
            t1[x] = {'exp': None, 'low': None, 'high': None, 'source': Source, 'name': x}

        if x in myexp2.keys():
            t2[x] = myexp2[x]
        else:
            t2[x] = {'exp': None, 'low': None, 'high': None, 'source': Source, 'name': x}
...
fh.write(
    '<a href="./tmp/%s-%s.png"><IMG SRC="./tmp/%s-%s.png" ismap usemap="#points" WIDTH="%d" HEIGHT="%d"></a><MAP name="points">' % (
    Nam1, Nam2, Nam1, Nam2, image_width, image_height))

for xc, yc, experiment, paper, description, gene1exp, gene2exp in zip(xcoords, ycoords, myname2, mysource, mydesc, myx,
                                                                      myy):
    # fh.write( "\n" + experiment + "\n" + paper + "\n" + description +  "\n" + str(gene1exp) + "\n" + str(gene2exp) + "\n" )
